        ...
        if (this.state.hasError) {
            return (
                <div className="p-4 text-red-600 bg-red-50 border border-red-200 rounded-md">
                    <h3 className="font-bold">Something went wrong in the dialog.</h3>
                    <p className="text-sm mt-1">{this.state.error?.message}</p>
                    <Button variant="outline" size="sm" className="mt-2" onClick={() => this.setState({ hasError: false, error: null })}>
                        Try Again
                    </Button>
                </div>
            );
